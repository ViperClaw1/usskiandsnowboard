...
export const DistributionCharts = () => {
  const { data: athletesBySport = [] } = useQuery({
    queryKey: ['athletes-by-sport'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('athletes_by_sport')
        .select('*')
        .limit(6);
  ...
  const { data: employersByIndustry = [] } = useQuery({
    queryKey: ['employers-by-industry'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('employers_by_industry')
        .select('*')
        .limit(6);
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                ...
                dataKey="count"
                nameKey="sport_discipline"
                cx="50%"
                cy="50%"
                outerRadius={80}
                label={(entry) => entry.sport_discipline}
              >
                {athletesBySport.map((_, index) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle>Employers by Industry</CardTitle>
